#include "Assembler.hpp"

#include "Constructs.hpp"
#include "MemManip.hpp"

#include <magic_enum/magic_enum.hpp>
#include <libcoro/Generator.hpp>
#include <liberror/Try.hpp>

#include <algorithm>
#include <cstring>
#include <functional>
#include <regex>
#include <sstream>
#include <string>
#include <fstream>

using namespace liberror;
using namespace libcoro;

static std::map<std::string, int32_t> codeSegmentOffsets_g;

static Generator<std::string> next_line(std::string_view code)
{
    std::istringstream stream(code.data());

    for (std::string line; std::getline(stream, line); )
    {
        co_yield line;
    }

    co_return;
}

static Generator<std::string> next_line(std::filesystem::path path)
{
    std::ifstream stream(path);

    for (std::string line; std::getline(stream, line); )
    {
        co_yield line;
    }

    co_return;
}

static std::string read_source_file(std::filesystem::path path)
{
    std::string source {};

    for (std::string_view prefix = ""; auto line : next_line(path))
    {
        source += prefix;
        source += line;
        prefix = "\n";
    }

    return source;
}

static Result<std::vector<uint8_t>> assemble_data_segment(std::string_view code)
{
    std::vector<uint8_t> bytes {};

    std::istringstream stream(code.data());
    std::string segment; stream >> segment;

    if (segment != ".DATA")
    {
        return make_error("Unexpected segment '{}' was reached", segment);
    }

    while (!stream.eof())
    {
        int size; stream >> size;

        std::ignore = stream.get();

        std::string data; std::getline(stream, data);

        std::ranges::copy(int_2_bytes(size), std::back_inserter(bytes));
        std::ranges::transform(data, std::back_inserter(bytes), std::identity{});
    }

    return bytes;
}

static std::array<uint8_t, 3> assemble_push_a(std::string const& code)
{
    int value;
    std::string segment;
    std::stringstream(code) >> value >> segment;

    assert(magic_enum::enum_contains<DataSource>(segment));

    return {
        static_cast<uint8_t>(Opcode::PUSHA),
        static_cast<uint8_t>(value),
        static_cast<uint8_t>(*magic_enum::enum_cast<DataSource>(segment)),
    };

}

static std::array<uint8_t, 4> assemble_push_b(std::string const& code)
{
    std::regex pattern(R"((\[\.?(\w+) \+ (\w)\]) (\w+))");
    std::smatch match;
    std::regex_match(code, match, pattern);

    int offset;
    std::stringstream(match.str(3)) >> offset;

    assert(magic_enum::enum_contains<DataSource>(match.str(2)));
    assert(magic_enum::enum_contains<DataSource>(match.str(4)));

    return {
        static_cast<uint8_t>(Opcode::PUSHB),
        static_cast<uint8_t>(*magic_enum::enum_cast<DataSource>(match.str(2))),
        static_cast<uint8_t>(offset),
        static_cast<uint8_t>(*magic_enum::enum_cast<DataSource>(match.str(4)))
    };
}

static std::array<uint8_t, 2> assemble_call_a(std::string const& code)
{
    std::string value;
    std::stringstream(code) >> value;

    return {
        static_cast<uint8_t>(Opcode::CALLA),
        static_cast<uint8_t>(codeSegmentOffsets_g.at(value))
    };
}

static std::array<uint8_t, 2> assemble_call_b(std::string code)
{
    std::transform(code.begin(), code.end(), code.begin(), ::toupper);

    assert(magic_enum::enum_contains<Intrinsic>(code));

    return {
        static_cast<uint8_t>(Opcode::CALLB),
        static_cast<uint8_t>(*magic_enum::enum_cast<Intrinsic>(code))
    };
}

static std::array<uint8_t, 1> assemble_ret()
{
    return {
        static_cast<uint8_t>(Opcode::RET)
    };
}

static std::array<uint8_t, 2> assemble_pop(std::string const& code)
{
    return {
        static_cast<uint8_t>(Opcode::POP),
        static_cast<uint8_t>(*magic_enum::enum_cast<Intrinsic>(code))
    };
}

static Result<std::vector<uint8_t>> assemble_code_segment(std::string_view code)
{
    std::vector<uint8_t> bytes {};

    auto reader = next_line(code);

    if (auto segment = reader.next(); segment != ".CODE")
    {
        return make_error("Unexpected segment '{}' was reached", segment);
    }

    reader.next();

    for (auto header : reader)
    {
        assert(header.starts_with("FUNCTION") || header.starts_with("ENTRYPOINT"));

        codeSegmentOffsets_g.insert({ header.starts_with("FUNCTION") ? header.substr(header.find_first_of(' ')+1) : "ENTRYPOINT", bytes.size() });

        reader.next();

        for (auto instruction : reader)
        {
            if (instruction.empty()) break;

            auto opcode = instruction.substr(0, instruction.find_first_of(' '));

            if (opcode != "RET")
            {
                auto operands = instruction.substr(instruction.find_first_of(' ') + 1);

                if      (opcode == "PUSHA") std::ranges::copy(assemble_push_a(operands), std::back_inserter(bytes));
                else if (opcode == "PUSHB") std::ranges::copy(assemble_push_b(operands), std::back_inserter(bytes));
                else if (opcode == "CALLA") std::ranges::copy(assemble_call_a(operands), std::back_inserter(bytes));
                else if (opcode == "CALLB") std::ranges::copy(assemble_call_b(operands), std::back_inserter(bytes));
                else if (opcode == "POP")   std::ranges::copy(assemble_pop(operands), std::back_inserter(bytes));
            }
            else
            {
                std::ranges::copy(assemble_ret(), std::back_inserter(bytes));
            }
        }
    }

    return bytes;
}

Result<std::vector<uint8_t>> assemble(std::filesystem::path source)
{
    auto code = read_source_file(source);

    std::stringstream stream(code);

    std::string dataSegment;
    std::string codeSegment;

    for (auto bytes = 0zu; !stream.eof(); )
    {
        std::string line; std::getline(stream, line);

        if (line.starts_with('.') && bytes > 0)
        {
            dataSegment = code.substr(0, bytes);
            codeSegment = code.substr(bytes + code.substr(bytes).find_first_of('.'));

            break;
        }

        bytes += line.size();
        bytes += line.empty() ? 0 : 1;
    }

    auto dataSegmentBytes = TRY(assemble_data_segment(dataSegment));
    auto codeSegmentBytes = TRY(assemble_code_segment(codeSegment));

    std::vector<uint8_t> program {};

    std::ranges::copy("This is a kubo program", std::back_inserter(program));
    program.pop_back();

    // dataSegmentStart offset
    std::ranges::copy(int_2_bytes(0), std::back_inserter(program));
    // codeSegmentStart offset
    std::ranges::copy(int_2_bytes(static_cast<int32_t>(dataSegmentBytes.size())), std::back_inserter(program));
    // entrypoint offset
    std::ranges::copy(int_2_bytes(codeSegmentOffsets_g.at("ENTRYPOINT")), std::back_inserter(program));

    std::ranges::copy(dataSegmentBytes, std::back_inserter(program));
    std::ranges::copy(codeSegmentBytes, std::back_inserter(program));

    return program;
}
